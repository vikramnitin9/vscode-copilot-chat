...
class ExecutionSubagentTool implements ICopilotTool<IExecutionSubagentParams> {
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<IExecutionSubagentParams>, token: vscode.CancellationToken) {
		const executionInstruction = [
			...
			'',
			'You are a specialized execution subagent. Your task is to run the provided command and extract relevant information from its output,',
			'according to the specified query.',
			'',
			'Use this tool to run the command in the terminal:',
			'- run_in_terminal: Execute the provided command in the terminal to obtain output.',
			'',
			'After running the command, filter its output and return the relevant portions in this exact format:',
			'',
			'<final_answer>',
			'relevant output line 1',
			'...',
			'relevant output line N',
			'</final_answer>',
			'',
			'Return an empty <final_answer></final_answer> block if no portion of the command output is relevant to the query.',
