	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='toolUseInstructions'>
				...
				NEVER say the name of a tool to a user. For example, instead of saying that you'll use the {ToolName.ExecutionSubagent} sub-agent, say "I'll run the command in a terminal".<br />
				{tools[ToolName.SearchSubagent] && <>For any context searching, use {ToolName.SearchSubagent} to search and gather data instead of directly calling {ToolName.FindTextInFiles}, {ToolName.Codebase} or {ToolName.FindFiles}.<br /></>}
				...
				{tools[ToolName.ExecutionSubagent] && <>Don't call the {ToolName.ExecutionSubagent} or the {ToolName.CoreRunInTerminal} tool multiple times in parallel. Instead, run one command and wait for the output before running the next command.<br /></>}
				...
				{tools[ToolName.ExecutionSubagent] && <>NEVER try to edit a file by running terminal commands unless the user specifically asks for it.<br /></>}
	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='outputFormatting'>
				...
				{isGpt5 && <>
					{tools[ToolName.ExecutionSubagent] ? <>
						When commands are required, call {ToolName.ExecutionSubagent} yourself and summarize the results. Do not print runnable commands unless the user asks. If you must show them for documentation, make them clearly optional and keep one command per line.<br />
					</> : <>
